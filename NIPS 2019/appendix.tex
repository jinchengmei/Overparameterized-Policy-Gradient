...
\begin{equation*}
\begin{split}
	\left\| \frac{d\tilde{\ell}_t}{d \rvw_r(t)} \right\|_2 \ge c \cdot \left( \max\limits_{k \in \left[h\right]}\left\{ \hat{r}_t\left(k\right) \right\} - \rvpi\left( \rmW_t \right)^\top \hat{\rvr}_t \right) .
\end{split}
\end{equation*}
\begin{proof}
...
\begin{equation*}
	\rvp_{k^\prime, r} \triangleq \sum\limits_{k=1}^{h}{ \left[ \hat{r}_{t}\left(k\right) \cdot \pi_{t}\left(k\right) \cdot v_{k^\prime,k}(t) \cdot \sI\left\{ \rvw_r(0)^\top \rvs > 0 \right\} \cdot \rvs \right] },
\end{equation*}
...
\begin{split}
	...
	&=  \sum\limits_{k=1}^{h}{ \left[ \hat{r}_{t}\left(k\right) \cdot \pi_{t}\left(k\right) \cdot v_{\hat{k}^*,k}(t) \cdot \sigma( \rvw_r(0)^\top \rvs ) \right] }.
\end{split}
\end{equation}
Denote $x \triangleq \rvw_r(0)^\top \rvs \sim \gN(0, \sigma^2)$. Note that $h_{\hat{k}^*,r}$ is a convex function of $x$. Moreover,
\begin{equation*}
\begin{split}
	\sum\limits_{k=1}^{h}{ \left[ \hat{r}_{t}\left(k\right) \cdot \pi_{t}\left(k\right) \cdot v_{\hat{k}^*,k}(t) \right] } &= \hat{r}_{\hat{k}^*}\left(t\right) \cdot \pi_{t}(\hat{k}^*) \cdot v_{\hat{k}^*,\hat{k}^*}(t) + \sum\limits_{k\not=\hat{k}^*}{ \left[ \hat{r}_{t}\left(k\right) \cdot \pi_{t}\left(k\right) \cdot v_{\hat{k}^*,k}(t) \right] } \\
	&= \hat{r}_{\hat{k}^*}\left(t\right) \cdot \pi_{t}(\hat{k}^*) \cdot \left( 1 - \pi_{t}(\hat{k}^*) \right) - \pi_{t}(\hat{k}^*) \cdot \sum\limits_{k\not=\hat{k}^*}{ \left[ \hat{r}_{t}\left(k\right) \cdot \pi_{t}\left(k\right) \right] } \\
	&= \pi_{t}(\hat{k}^*) \cdot \left( \hat{r}_{\hat{k}^*}\left(t\right) - \sum\limits_{k=1}^{h}{ \left[ \hat{r}_{t}\left(k\right) \cdot \pi_{t}\left(k\right) \right] } \right) \\
	&= \pi_{t}(\hat{k}^*) \cdot \left( \max\limits_{k \in \left[h\right]}\left\{ \hat{r}_t\left(k\right) \right\} - \rvpi\left( \rmW_t \right)^\top \hat{\rvr}_t  \right).
\end{split}
\end{equation*}
By assumption, if $\pi_{t}(\hat{k}^*) > c > 0$, then ,
\begin{equation*}
\begin{split}
	\left| \partial{h_{\hat{k}^*,r}}_{\max}{(0)} - \partial{h_{\hat{k}^*,r}}_{\min}{(0)} \right| &= \left| \sum\limits_{k=1}^{h}{ \left[  \hat{r}_{t}\left(k\right) \cdot \pi_{t}\left(k\right) \cdot v_{\hat{k}^*,k}(t) \right] } \right| \\
	&= \pi_{t}(\hat{k}^*) \cdot \left( \max\limits_{k \in \left[h\right]}\left\{ \hat{r}_t\left(k\right) \right\} - \rvpi\left( \rmW_t \right)^\top \hat{\rvr}_t  \right) \\
