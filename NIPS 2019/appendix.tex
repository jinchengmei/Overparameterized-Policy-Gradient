...
We conduct experimental results to compare the proposed two algorithms. The setting is $h = 50$, $m = 5000$, $d = 10$. We repeatedly run $10$ experiments. In each experiment, the true mean reward and variance are randomly generated. LLE significantly outperforms PGE as shown in the \cref{fig:regret}.

\begin{figure}[h]
	%\vskip 0.2in
	\begin{center}
		\centerline{\includegraphics[width=0.6\columnwidth]{CumulativeRegret.pdf}}
		\caption{The cumulative regret of PGE and LLE.}
		\label{fig:regret}
	\end{center}
	\vskip -0.2in
\end{figure}
