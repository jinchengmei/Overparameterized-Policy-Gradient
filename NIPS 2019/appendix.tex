...
\begin{equation*}
\begin{split}
	\sum\limits_{k=1}^{h}{ \left[ \hat{r}_{k}\left(t\right) \cdot \pi_{k}(t) \cdot v_{\hat{k}^*,k}(t) \right] } &= \hat{r}_{\hat{k}^*}\left(t\right) \cdot \pi_{\hat{k}^*}(t) \cdot v_{\hat{k}^*,\hat{k}^*}(t) + \sum\limits_{k\not=\hat{k}^*}{ \left[ \hat{r}_{k}\left(t\right) \cdot \pi_{k}(t) \cdot v_{\hat{k}^*,k}(t) \right] } \\
	...
	&= \pi_{\hat{k}^*}(t) \cdot \left( \hat{r}_{\hat{k}^*}\left(t\right) - \sum\limits_{k=1}^{h}{ \left[ \hat{r}_{k}\left(t\right) \cdot \pi_{k}(t) \right] } \right) \\
