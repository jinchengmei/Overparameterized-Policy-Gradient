...
\begin{proof}
	...
	\end{split}
	\end{equation*}
	Denote $\bar{t}(k) \triangleq \max\left\{ t_1(k), t_2, t_3 + t_4 \right\} = \max\left\{ t_1, t_3 + t_4 \right\} $. After $\bar{t}(k)$ steps, with high probability, $\tilde{\rvpi}_t$ will coincide with the optimal policy. Finally, for $\alpha = 3$ and $\beta = 256$, by \cref{lem:delta_estimation_lower_bound},
	\begin{equation*}
	\begin{split}
