...
\subsection{Proof of \cref{thm:logit_learning_main_result} (Theorem 2 in the paper)}
...
\begin{split}
    ...
    &\quad \le - \frac{1}{2 h} \cdot \frac{1}{2} \left\| \rvo\left( \rmW_t\right) - \hat{\rvr}_t \right\|_2^2. \quad (\text{using similar proof idea as \cref{lem:gradient_lower_bound}} ) \qedhere
\end{split}
\end{equation*}
\end{proof}
...
We conduct experiments on the proposed algorithms. The setting is $h = 50$, $m = 5000$, $d = 20$, $T = 100000$. The true mean reward vector $\rvr \in [0.0, 1.0]^h$. We repeatedly run $10$ experiments. In each experiment, the true mean reward and variance are randomly generated. For PGE, $\frac{1}{3} - \beta$ is set to be $0.33$, $\sigma = 0.002$, and $\eta = 0.001$. For LLE, $\varepsilon_t = \frac{\ln{t}}{t}$, $\sigma = 0.002$, and $\eta = 0.001$. At each step $t$, we record the expected regret $\sum\limits_{s = 0}^{t - 1}{ \left( {\rvpi^*} ^\top \rvr - \expectation{\left[ r(A_s) \right]} \right) } $ and the latest expected loss $ {\rvpi^*} ^\top \rvr - \expectation{\left[ r(A_t) \right]} $, where the expectation is taken over the mixed sampling policy in PGE/LLE.

As shown in \cref{fig:regret}, both PGE and LLE have expected regrets sublinear in $T$. On the other hand, the expected regret of PGE decays much slower than that of LLE, since PGE has a large budget $t^{\beta - \frac{1}{3}}$ for uniform exploration.

\begin{figure}[h]
	%\vskip 0.2in
	\begin{center}
		\centerline{\includegraphics[width=1.0\columnwidth]{comparison_PGE_LLE.pdf}}
