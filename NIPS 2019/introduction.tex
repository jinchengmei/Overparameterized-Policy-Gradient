...
\begin{itemize}
	\item We propose a policy based RL method, Policy Gradient with $\varepsilon$-Greedy Exploration (PGE), that is similar to the vanilla policy gradient method, but with an $\varepsilon$-greedy exploration and a momentum on the rewards. We prove that PGE, with over-parametrization neural network as its policy network, enjoys a sub-linear convergence rate under the stochastic bandit setting.
	\item We further propose a value based RL method, Logit Learning with $\varepsilon$-Greedy Exploration (LLE), which, again with over-parametrization neural network as its value network, achieves a nearly optimal regret under the stochastic bandit setting. This algorithm learns value based objectives using the gradient descent. 
\end{itemize}
