...
As discussion in \cref{sec:policy_gradient}, our policy gradient method achieves an unsatisfactory $\tilde{O}(T^{2/3})$ regret.
...
\begin{remk}
	...
	This is due to the low efficiency of $\epsilon$-greedy exploration.
	In particular, to have a proper $\epsilon$, one needs to estimate the reward gaps $\Delta(k)$ which causes the extra $\ln T$ factor.
