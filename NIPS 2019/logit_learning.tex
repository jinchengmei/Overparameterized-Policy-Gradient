...
\begin{algorithm}[t]
	...
	\begin{algorithmic}
		...
		\STATE $\hat{\Delta}_{t}(k) \gets \min\left\{ 1,  \min\limits_{k^\prime \in \left[h\right]}\left\{ \lcb_{t}(k^\prime) \right\}  - \ucb_{t}(k)\right\} $, $\forall k \in [h]$.
...
We show, in the next theorem, that LLE has a nearly optimal regret rate in the stochastic multi-armed bandit setting, when the value network is over-parametrized.
\begin{thm}
