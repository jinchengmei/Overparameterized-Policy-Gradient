...
\begin{split}
    ...
    &\le \left( {\rvpi_t^*} - \rvpi\left( \rmW_t \right) \right)^\top \hat{\rvr}_t + 2 \left( 2 + \left( h \ln{t} \right) ^\frac{1}{3} \right) \cdot t^{- \frac{1}{3}},
\end{split}
\end{equation}
...
\label{thm:reward_estimation_hoeffding}
    In \cref{alg:policy_gradient_uniform_exploration}, let $\beta = \frac{ \ln{\left(\frac{h}{6}\right) + \ln{\ln{t}} } }{ 3 \ln{t}}$. $\forall t \ge \frac{h}{6} \ln{\left(\frac{h}{6}\right) }$, the following holds in expectation,
\begin{equation*}
    \left\| \hat{\rvr}_t - \rvr \right\|_\infty \le 2 \cdot t^{- \frac{1}{3}} + t^{\beta - \frac{1}{3}} \le \left( 2 + \left( h \ln{t} \right) ^\frac{1}{3} \right) \cdot t^{- \frac{1}{3}}
\end{equation*}
\end{thm}
\begin{proof}
    The expected sampled number of any action $k$ up to step $t$ can be lower bounded,
\begin{equation*}
\begin{split}
