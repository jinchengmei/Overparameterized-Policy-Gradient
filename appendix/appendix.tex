...
\begin{split}
    ...
    &\qquad + \sum\limits_{t=0}^{T-1}{ \left( {\rvpi_i^*(t)} - {\rvpi_i^*} \right)^\top \hat{\rvr}_i(t) } \\
    &\qquad + \sum\limits_{t=0}^{T-1}{ \rvpi_i\left(\rmW(t)\right)^\top \left( \rvr_i - \hat{\rvr}_i(t) \right) } + \sum\limits_{t=0}^{T-1}{ {\rvpi_i^*}^\top \left( \hat{\rvr}_i(t) - \rvr_i \right) },
\end{split}
\end{equation}
...
\begin{split}
    ...
    &\quad + {\rvpi_i^*(t-1)}^\top \hat{\rvr}_i(t-1) - {\rvpi_i^*(t-1)}^\top \hat{\rvr}_i(t) \\
    &\quad + {\rvpi_i^*(t-1)}^\top \hat{\rvr}_i(t) - {\rvpi_i^*(t)}^\top \hat{\rvr}_i(t).
\end{split}
\end{equation}
...
\begin{split}
    {\rvpi_i^*(t-1)}^\top \hat{\rvr}_i(t-1) - {\rvpi_i^*(t-1)}^\top \hat{\rvr}_i(t) \le \frac{1}{t}.
\end{split}
\end{equation}
Plugging \cref{eq:first_term_upper_bound}, \cref{eq:second_term_upper_bound}, and \cref{eq:fourth_term_upper_bound} into \cref{eq:dynamic_regret_decomposition},
\begin{equation*}
\begin{split}
    \omega_t &\le \frac{1}{t} - \frac{\delta^2 c^2}{8 h n^4} \cdot \omega_{t-1}^2 + \omega_{t-1} + \frac{1}{t} + {\rvpi_i^*(t-1)}^\top \hat{\rvr}_i(t) - {\rvpi_i^*(t)}^\top \hat{\rvr}_i(t) \\
    &= - \frac{\delta^2 c^2}{8 h n^4} \cdot \omega_{t-1}^2 + \omega_{t-1} + \frac{2}{t} + {\rvpi_i^*(t-1)}^\top \hat{\rvr}_i(t) - {\rvpi_i^*(t)}^\top \hat{\rvr}_i(t).
\end{split}
\end{equation*}
Rearranging and summing up from $t = 1$ to $T$,
\begin{equation*}
\begin{split}
    \sum\limits_{t=1}^{T}{ \omega_{t-1}^2 } &\le \frac{8 h n^4}{\delta^2 c^2} \sum\limits_{t=1}^{T}{\left[ \omega_{t-1} - \omega_t + \frac{2}{t} + {\rvpi_i^*(t-1)}^\top \hat{\rvr}_i(t) - {\rvpi_i^*(t)}^\top \hat{\rvr}_i(t) \right]} \\
    &= \frac{8 h n^4}{\delta^2 c^2} \left[ \omega_0 - \omega_T + \sum\limits_{t=1}^{T}{\frac{2}{t}} + \sum\limits_{t=1}^{T}{\left( {\rvpi_i^*(t-1)}^\top \hat{\rvr}_i(t) - {\rvpi_i^*(t)}^\top \hat{\rvr}_i(t) \right)} \right] \\
    &\le \frac{8 h n^4}{\delta^2 c^2} \left( 5 + \log{T} + T^a \right).
\end{split}
\end{equation*}
By the the Root-Mean Square-Arithmetic Mean inequality,
\begin{equation*}
\begin{split}
    \sum\limits_{t=0}^{T-1}{ \omega_t } &\le \sqrt{ T \cdot \sum\limits_{t=0}^{T-1}{ \omega_{t}^2 } } \\
    &\le \frac{4 n^2 \sqrt{h}}{\delta c}\sqrt{T\cdot\left( \log{T} + T^a \right)}
\end{split}
\end{equation*}
\end{proof}
