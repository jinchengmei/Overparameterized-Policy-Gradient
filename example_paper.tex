...
In the RL side, it is well studied that under bandit setting and tabular case of Markov decision process (MDP) setting, many popular algorithms enjoy theoretical guarantees. In particular, either the algorithm converges asymptotically, or its regret of has finite time sublinear upper bound. However, once it goes beyond tabular cases, the theoretical guarantee becomes weak. For example, in linear function approximation, only fixed point property is know for the algorithm. Even worse, with non-linear function approximation, like neural neural used in DQN algorithm, there is no theoretical analysis.
