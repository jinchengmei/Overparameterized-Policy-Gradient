...
\end{split}
\end{equation*}
for all the $t \ge \sqrt{T}$. Denote $\varepsilon \triangleq \frac{n^4}{\eta m c^2 \delta^2 \sqrt{T}}$, the regret can be upper bounded by the sum of two parts,
\begin{equation*}
\begin{split}
    \sum\limits_{t=1}^{T}{ \rvpi_i\left( \rmW(\sqrt{T}) \right)^\top \rvtilder_i } &\le t \cdot 1 + \left(T - \sqrt{T} \right) \cdot \varepsilon \\
