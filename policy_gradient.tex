...
\begin{lem}
    Given weights $\rmW \triangleq \left[ \rvw_1, \rvw_2, \dots, \rvw_m \right]$, and $\rmW^\prime \triangleq \left[ \rvw_1^\prime, \rvw_2^\prime, \dots, \rvw_m^\prime \right]$. Denote $\rvpi_i\left( \rmW \right)$ and $\rvpi_i\left( \rmW^\prime \right)$ as the corresponding neural network policies.
\begin{equation*}
\begin{split}
    \rvpi\left( \rmW^\prime \right)^\top \rvtilder_i &\le \rvpi\left( \rmW \right)^\top \rvtilder_i + \left\langle \frac{d \rvpi_i\left( \rmW \right)^\top \rvtilder_i }{d\left( \rmW \right)}, \rmW^\prime - \rmW \right\rangle \\
    &\quad + 4 \sqrt{m} \cdot \rvpi_i\left( \rmW \right)^\top \rvtilder_i \cdot \left\| \rmW^\prime - \rmW \right\|_F
\end{split}
\end{equation*}
\end{lem}
\begin{proof}
...
\begin{equation*}
\begin{split}
    \left| \rvpi\left( \rmW^\prime \right)^\top \rvtilder_i - \rvpi\left( \rmW \right)^\top \rvtilder_i \right| &= \left| \int_{0}^{1} { \frac{ d \rvpi_i\left( \rmW +  \xi \cdot\left( \rmW^\prime - \rmW \right) \right)^\top \rvtilder_i }{d\xi} } d\xi \right| \\
    ...
    &\le \int_{0}^{1}{  \sqrt{ \sum\limits_{r=1}^{m}{\left( 4 \cdot \rvpi_i\left( \bar{\rmW}_\xi \right)^\top \rvtilder_i \right)^2 } } \cdot \left\| \rmW^\prime - \rmW \right\|_F d\xi} \\
    &= 4 \sqrt{m} \cdot \rvpi_i\left( \bar{\rmW}_\xi \right)^\top \rvtilder_i \cdot \left\| \rmW^\prime - \rmW \right\|_F
\end{split}
\end{equation*}
...
\begin{split}
    ...
    &\le 4 \sqrt{m} \cdot \rvpi_i\left( \rmW \right)^\top \rvtilder_i \cdot \left\| \rmW^\prime - \rmW \right\|_F  \qedhere
\end{split}
\end{equation*}
\end{proof}
