...
\begin{equation*}
\begin{split}
	\left\| \frac{d\tilde{\ell}}{d \rvw_r(t)} \right\|_2 \ge \Omega\left( \frac{\delta}{n^2} \right) \cdot \regret\left( \rvpi_{i^*(t)}(t) \right) \ge \Omega\left( \frac{\delta}{n^3} \right) \cdot \regret\left( \rvpi(t) \right).
\end{split}
\end{equation*}
\end{lem}
\begin{proof}
...
\begin{equation*}
\begin{split}
	\probability\limits_{x \sim  \gU[-\tau, \tau]}\left\{ h_{k^*,r} \ge \frac{c\tau}{8n^2} \cdot \regret(\rvpi(t)) \right\} > \frac{1}{8}.
\end{split}
\end{equation*}
Therefore, we have,
\begin{equation}
...
\begin{split}
	\probability\left\{ h_{k^*,r} \ge \frac{c\tau}{8n^2} \cdot \regret(\rvpi(t)) \right\} &\ge \probability\left\{ h_{k^*,r} \ge \frac{c\tau}{8n^2} \cdot \regret(\rvpi(t)) \middle| \gE_1 \land \gE_2 \right\} \cdot \probability\left\{ \gE_1 \land \gE_2 \right\} \\
	&\ge \frac{1}{8} \cdot \frac{5\sqrt{2}}{6\sqrt{\pi}} \cdot \frac{\tau}{\sigma} \cdot \frac{1}{\sqrt{\pi}} = \frac{5\sqrt{2}}{48\pi} \cdot \frac{\tau}{\sigma}.
\end{split}
\end{equation}
Finally, $h_{k^*,r} \sim \gN\left( 0, \left\| \rvp_{k^*, r} \right\|_2^2 \cdot \sigma^2 \right)$, with probability at least $1 - \frac{\delta \sigma}{n}$,
\begin{equation}
\label{eq:p_h_lower_bound}
\begin{split}
	h_{k^*,r} < \sqrt{ 2  \log{ \left( \frac{n}{ \delta \sigma} \right) }} \cdot \left\| \rvp_{k^*, r} \right\|_2 \cdot \sigma
\end{split}
\end{equation}
...
\begin{split}
	...
	&> \frac{1}{ \sqrt{ 2  \log{ \left( \frac{n}{ \delta \sigma} \right) }} } \cdot \frac{h_{k^*,r}}{\sigma} \\
	&\ge \frac{1}{ \sqrt{ 2  \log{ \left( \frac{n}{ \delta \sigma} \right) }} } \cdot \frac{c\tau}{8\sigma n^2} \cdot \regret(\rvpi(t)),
\end{split}
\end{equation*}
with probability at least $\left( \frac{1}{2} - \frac{1}{\sqrt{2\pi}} \right) \cdot \left( 1 - \frac{\delta \sigma}{n} \right) \cdot \frac{5\sqrt{2}}{48\pi} \cdot \frac{\tau}{\sigma}$, where $\tau = \frac{\delta\sigma}{2n}$.
\end{proof}
...
\end{lem}
\begin{proof}
	Denote $\rho = \partial\phi_{\max}{(0)} - \partial\phi_{\min}{(0)}$, we have $\partial\phi_{\max}{(0)} \ge \frac{\rho}{2}$ or $\partial\phi_{\min}{(0)} \le - \frac{\rho}{2}$. If $\partial\phi_{\min}{(0)} \le - \frac{\rho}{2}$, then $-\phi$ will satisfy the first case. So we prove for $\partial\phi_{\max}{(0)} \ge \frac{\rho}{2}$. Denote $\hat{\phi}(x) = \phi(x) - \phi(0)$. $\forall x > 0$,
\begin{equation*}
\begin{split}
